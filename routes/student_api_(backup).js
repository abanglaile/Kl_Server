...
function insertBreakdownLog(logid, breakdown_sn, exercise_id, kp_rating){
    var sql = "";
    var params = [];
    for(var j = 0; j < breakdown_sn.length; j++){
        if(breakdown_sn[j].sn_state >= 0){
            sql += "insert into breakdown_log(logid, sn, kpid, sn_state, kp_old_rating, kp_delta_rating, sn_old_rating, sn_delta_rating) values(?, ?, ?, ?, ?, ?, ?, ?);";
            // params = [
            //             logid, 
            //             breakdown_sn[j].sn, 
            //             breakdown_sn[j].kpid, 
    ...
    if(req.body.student_id && req.body.student_rating && req.body.test_result){
        ...
        for(var i = 0; i < exercise_log.length; i++){
            ...
            insertExerciseLog(student_id, test_result.test_id, log, results => {
                ...
                insertBreakdownLog(logid, log.breakdown_sn, log.exercise_id);
            });
            // //插入每条题目rating变化记录
            // sql += "insert into exercise_delta_rating(exercise_id, delta_rating) values(?, ?);";
